...
(() => {
 ...
 grid.innerHTML=list.map(p=>{const packs=p.packSizes||[],sel=S.packs.get(p.name)||packs[0]||null;return `<article class="kt-card"><img src="${esc(p.images?.[0]||'')}" alt="${esc(p.name)}" loading="lazy"><div class="kt-card-body"><div class="kt-card-name">${esc(p.name)}</div><div class="kt-card-cat">${esc(p.category)}</div>${(Number(p.minQty)||1)>1?`<div class="kt-moq">Minimum order: ${Number(p.minQty)} units</div>`:''}${packs.length?`<div class="kt-packs">${packs.map((x,i)=>`<button type="button" class="kt-pack ${sel===x?'active':''}" data-name="${esc(p.name)}" data-i="${i}">${esc(x.size)}${x.price?' · '+esc(x.price):''}</button>`).join('')}</div>`:''}<button type="button" class="kt-add" data-name="${esc(p.name)}" ${p.inStock===false?'disabled':''}>${p.inStock===false?'Out of Stock':'Add to Order'}</button></div></article>`}).join('');
 grid.querySelectorAll('.kt-pack').forEach(b=>b.onclick=()=>{const p=products().find(x=>x.name===b.dataset.name);if(p)S.packs.set(p.name,(p.packSizes||[])[+b.dataset.i]);render()});
 grid.querySelectorAll('.kt-add').forEach(b=>b.onclick=()=>{const p=products().find(x=>x.name===b.dataset.name);if(!p)return;window.addToCart(p.name,S.packs.get(p.name)||(p.packSizes||[])[0]||null,false);bar();b.textContent='Added ✓';setTimeout(()=>{if(document.body.contains(b))b.textContent='Add to Order'},900)});bar()};
 const init=()=>{const s=document.getElementById('product-search');if(s){s.oninput=null;s.removeAttribute('oninput');s.addEventListener('input',()=>{S.search=s.value;render()})}const st=document.getElementById('kt-category-strip');document.getElementById('kt-cat-prev').onclick=()=>st.scrollBy({left:-240,behavior:'smooth'});document.getElementById('kt-cat-next').onclick=()=>st.scrollBy({left:240,behavior:'smooth'});bar();const t=setInterval(()=>{if(typeof allProducts!=='undefined'&&allProducts.length){clearInterval(t);render()}},200)};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
})();
EOF
