...
if 'openDrawer = true' not in cs or 'if (openDrawer) openCart();' not in cs: raise SystemExit('cart patch verification failed')
c.write_text(cs)
PY
...
(() => {
 ...
 const bar=()=>{const n=countCart(),e=document.querySelector('#kt-order-bar #kt-order-count');if(e)e.textContent=`${n} product${n===1?'':'s'}`};
