...
`;document.head.appendChild(s);}
  ...
  function computeActiveSection(){if(state.category!=='All'){if(state.activeSection!==null){state.activeSection=null;}refreshActivePill();return;}const root=document.getElementById('kt-home-catalogue');if(!root)return;const dividers=[...root.querySelectorAll('.kt-cat-divider')];if(!dividers.length)return;const nav=document.getElementById('kt-category-nav');const offset=(nav?nav.getBoundingClientRect().bottom:0)+6;let current='All';for(const d of dividers){if(d.getBoundingClientRect().top<=offset)current=d.dataset.cat;else break;}if(current!==state.activeSection){state.activeSection=current;refreshActivePill();}}
