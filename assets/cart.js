...
// Cloudinary-hosted product photos are served at whatever resolution they
// were uploaded at, with no resize applied -- the browser was downloading
// full-size originals just to paint a 280px-wide card thumbnail. Cloudinary
// supports resizing/compressing on the fly via URL segments, so this is a
// non-destructive view of the same asset (the original upload is untouched).
// Non-Cloudinary images (plain files under /images/) pass through unchanged
// since there's no transform service in front of them.
...
function renderCart() {
    ...
    list.innerHTML = cart.map((item, idx) => {
        ...
        return `<div class="flex items-center gap-2 bg-gray-50 border border-gray-100 p-2">
            ${item.image ? `<img src="${cldOptimize(item.image,150)}" class="w-12 h-12 object-cover flex-shrink-0 border border-gray-200">` : '<div class="w-12 h-12 bg-gray-200 flex-shrink-0 flex items-center justify-center"><i class="fas fa-box-open text-gray-300 text-sm"></i></div>'}
...
(function initSingleHomepageCatalogue(){
  ...
  if(!isHome()) return;
  const state={category:'All',query:'',qty:Object.create(null),installed:false,activeSection:'All'};
...
#kt-catalogue-wrap{max-width:1280px;margin:0 auto;padding:0 24px;text-align:left}#kt-catalogue-controls{padding-bottom:8px}#kt-category-nav{position:sticky;top:0;z-index:30;display:flex;align-items:center;gap:6px;background:#F6F6F6;padding:7px 0;border-bottom:1px solid #e5e7eb}#kt-category-strip{display:flex;gap:7px;overflow-x:auto;padding:4px 0 8px;scrollbar-width:none;scroll-behavior:smooth;flex:1;min-width:0}#kt-category-strip::-webkit-scrollbar{display:none}.kt-cat{flex:0 0 auto;border:1px solid #131313;background:#131313;color:#FFF200;border-radius:999px;padding:7px 13px;font:700 11px Manrope,sans-serif;cursor:pointer;transition:background-color .25s ease,color .25s ease,border-color .25s ease}.kt-cat:not(.active):hover{border-color:#FFF200}.kt-cat.active{background:#FFF200;color:#131313;border-color:#FFF200}.kt-cat-arrow{flex:0 0 34px;width:34px;height:34px;border:1px solid #d1d5db;background:#fff;color:#131313;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;font:800 16px/1 Manrope,sans-serif;transition:background .15s,border-color .15s,opacity .15s;touch-action:manipulation}.kt-cat-arrow:hover:not(:disabled){background:#f3f4f6;border-color:#9ca3af}.kt-cat-arrow:disabled{opacity:.28;cursor:default}.kt-cat-arrow[hidden]{display:none}@media(max-width:767px){#kt-category-nav{gap:5px}.kt-cat-arrow{flex-basis:32px;width:32px;height:32px}}
#kt-cold-chain-note{display:none;align-items:flex-start;gap:10px;margin:0 0 12px;padding:12px 16px;background:#f0f9ff;border:1px solid #bae6fd;color:#075985;font:600 12px/1.5 Manrope,sans-serif;border-radius:6px}#kt-cold-chain-note.show{display:flex}#kt-home-catalogue{padding:8px 0 110px;text-align:left}.kt-cat-section:first-child .kt-cat-divider{margin-top:2px}.kt-cat-divider{display:flex;align-items:center;gap:10px;margin:24px 0 12px;font:800 12px Archivo,Manrope,sans-serif;text-transform:uppercase;letter-spacing:.08em;color:#131313}.kt-cat-divider::before{content:'';width:4px;height:15px;background:#FFF200;flex:0 0 auto;border-radius:2px}.kt-cat-divider::after{content:'';flex:1;height:1px;background:#e5e7eb}.kt-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px;align-items:start}.kt-card{background:#fff;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;display:flex;flex-direction:column;min-width:0}.kt-card:hover{border-color:#FFF200;box-shadow:0 7px 20px rgba(0,0,0,.08)}.kt-card-img{width:100%;height:190px;object-fit:contain;background:#fff;display:block;padding:10px}.kt-no-image{display:flex;align-items:center;justify-content:center;color:#6b7280;font:600 12px Manrope,sans-serif;background:#f8f8f8}.kt-card-body{padding:12px;display:flex;flex-direction:column}.kt-card-meta{display:flex;justify-content:space-between;gap:8px;font:700 9px Manrope,sans-serif;text-transform:uppercase;letter-spacing:.07em;color:#4b5563;margin-bottom:7px}.kt-stock{color:#166534}.kt-stock.out{color:#b91c1c}.kt-card h3{font:800 15px/1.2 Archivo,Manrope,sans-serif;color:#131313;margin:0;min-height:36px}.kt-card-name-link{color:inherit;text-decoration:none}.kt-card-name-link:hover{color:#131313;text-decoration:underline}.kt-brand{font:600 10px Manrope,sans-serif;color:#4b5563;margin-top:4px;min-height:14px}.kt-desc{font:500 11px/1.4 Manrope,sans-serif;color:#4b5563;margin-top:6px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.kt-view-row{display:flex;gap:6px;margin-top:8px}.kt-view,.kt-share{flex:1;border:1px solid #d1d5db;background:#fff;color:#131313;border-radius:5px;padding:7px;font:700 10px Manrope,sans-serif;text-transform:uppercase;letter-spacing:.04em;cursor:pointer}.kt-view:hover,.kt-share:hover{border-color:#131313;background:#f7f7f7}.kt-share-cat{border:1px solid #d1d5db;background:#fff;color:#131313;border-radius:5px;padding:5px 10px;font:700 9px Manrope,sans-serif;text-transform:uppercase;letter-spacing:.04em;cursor:pointer;white-space:nowrap;flex:0 0 auto}.kt-share-cat:hover{border-color:#131313;background:#f7f7f7}.kt-packs{display:flex;flex-wrap:wrap;gap:5px;margin-top:10px}.kt-pack{border:1px solid #d1d5db;background:#fff;border-radius:5px;padding:7px 8px;font:700 10px/1.1 Manrope,sans-serif;color:#131313;cursor:pointer}.kt-pack:hover,.kt-pack.selected{border-color:#131313;background:#f7f7f7}.kt-pack span{display:block;color:#6b7280;font-size:9px;margin-top:2px}.kt-price{font:600 11px Manrope,sans-serif;color:#4b5563;margin-top:10px}.kt-buy-row{display:flex;gap:7px;margin-top:12px}.kt-qty{display:flex;align-items:center;border:1px solid #d1d5db;border-radius:5px;overflow:hidden;height:38px}.kt-qty-btn{width:31px;height:100%;border:0;background:#f3f4f6;font-size:17px;font-weight:800;cursor:pointer}.kt-qty-value{min-width:31px;text-align:center;font:800 12px Manrope,sans-serif}.kt-add{flex:1;height:38px;border:0;border-radius:5px;background:#131313;color:#fff;font:800 11px Manrope,sans-serif;cursor:pointer}.kt-add:hover{background:#000}.kt-add:disabled{opacity:.45;cursor:not-allowed}.kt-download{display:flex;align-items:center;justify-content:center;width:100%;height:38px;margin-top:12px;border:0;border-radius:5px;background:#131313;color:#fff;font:800 11px Manrope,sans-serif;text-transform:uppercase;letter-spacing:.04em;text-decoration:none;cursor:pointer}.kt-download:hover{background:#000}.kt-moq{font:700 9px Manrope,sans-serif;color:#92400e;text-transform:uppercase;letter-spacing:.05em;margin-top:6px}.kt-empty{padding:42px 20px;text-align:center;border:1px dashed #d1d5db;border-radius:8px;color:#6b7280;font:600 13px Manrope,sans-serif}.kt-loading{padding:60px 20px;text-align:center;color:#6b7280;font:600 13px Manrope,sans-serif}.kt-loading i{font-size:22px;color:#131313;display:block;margin-bottom:10px}
...
@media(max-width:1023px){.kt-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:767px){#kt-catalogue-wrap{padding:0 12px}#kt-catalogue-controls{padding-bottom:6px}#kt-home-catalogue{padding:6px 0 105px}.kt-cat-divider{margin:18px 0 10px;font-size:11px}.kt-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.kt-card-img{height:145px;padding:7px}.kt-card-body{padding:9px}.kt-card h3{font-size:13px;min-height:32px}.kt-desc{display:none}.kt-view,.kt-share{font-size:9px;padding:6px}.kt-share-cat{font-size:8px;padding:4px 8px}.kt-pack{padding:6px;font-size:9px}.kt-buy-row{gap:5px}.kt-qty-btn{width:27px}.kt-qty-value{min-width:26px}.kt-add{font-size:10px}#kt-order-bar{bottom:9px}.kt-toast{right:12px;bottom:70px}}
`;document.head.appendChild(s);}
  function ensureControls(){const search=document.getElementById('product-search');if(!search)return;if(!search.dataset.ktBound){search.dataset.ktBound='1';search.removeAttribute('oninput');search.setAttribute('aria-label','Search wholesale products');search.addEventListener('input',()=>{state.query=search.value.trim().toLowerCase();render();});}const source=document.getElementById('tiles-container');if(!document.getElementById('kt-catalogue-wrap')){const wrap=document.createElement('div');wrap.id='kt-catalogue-wrap';wrap.innerHTML='<div id="kt-catalogue-controls"><div style="display:flex;align-items:center;justify-content:space-between;gap:8px"><span style="font:800 10px Manrope,sans-serif;text-transform:uppercase;letter-spacing:.12em;color:#6b7280;padding:2px 0 3px">Browse categories</span><button type="button" id="kt-share-category" class="kt-share-cat" hidden>Share category</button></div></div><div id="kt-category-nav"><button type="button" class="kt-cat-arrow" id="kt-cat-prev" aria-label="Previous categories">‹</button><div id="kt-category-strip" role="tablist" aria-label="Product categories"></div><button type="button" class="kt-cat-arrow" id="kt-cat-next" aria-label="Next categories">›</button></div><div id="kt-cold-chain-note"><span>❄️</span><span>Cold Chain Items — not delivered outside Karachi due to cold storage maintenance requirements.</span></div><div id="kt-home-catalogue" aria-label="Wholesale product catalogue"></div>';search.closest('.mb-4')?.insertAdjacentElement('afterend',wrap);}if(source)source.classList.add('kt-source-hidden');const oldHeading=document.querySelector('#products h2');if(oldHeading)oldHeading.textContent='Wholesale Catalogue';const oldEyebrow=oldHeading?.previousElementSibling;if(oldEyebrow)oldEyebrow.textContent='Products';if(!document.getElementById('kt-order-bar')){const bar=document.createElement('div');bar.id='kt-order-bar';bar.innerHTML='<div><strong id="kt-order-products">0 products</strong><span> · </span><span id="kt-order-units">0 units</span></div><button type="button" class="kt-review">Review order →</button>';document.body.appendChild(bar);bar.querySelector('.kt-review').addEventListener('click',()=>openCart());}}
  function currentActiveCat(){return state.category!=='All'?state.category:(state.activeSection||'All');}
  function refreshActivePill(){const activeCat=currentActiveCat();const strip=document.getElementById('kt-category-strip');if(!strip)return;strip.querySelectorAll('.kt-cat').forEach(b=>{const isActive=b.dataset.cat.toLowerCase()===activeCat.toLowerCase();if(b.classList.contains('active')!==isActive){b.classList.toggle('active',isActive);if(isActive)b.scrollIntoView({behavior:'smooth',inline:'center',block:'nearest'});}});const coldNote=document.getElementById('kt-cold-chain-note');if(coldNote)coldNote.classList.toggle('show',activeCat.toLowerCase()==='vaccines');}
  function computeActiveSection(){if(state.category!=='All'){if(state.activeSection!==null){state.activeSection=null;}refreshActivePill();return;}const root=document.getElementById('kt-home-catalogue');if(!root)return;const dividers=[...root.querySelectorAll('.kt-cat-divider')];if(!dividers.length)return;if(dividers.length<2){if(state.activeSection!==dividers[0].dataset.cat){state.activeSection=dividers[0].dataset.cat;refreshActivePill();}return;}const nav=document.getElementById('kt-category-nav');const offset=(nav?nav.getBoundingClientRect().bottom:0)+6;let current=dividers[0].dataset.cat;for(const d of dividers){if(d.getBoundingClientRect().top<=offset)current=d.dataset.cat;else break;}if(current!==state.activeSection){state.activeSection=current;refreshActivePill();}}
  function syncStickyOffset(){const nav=document.querySelector('nav.sticky');const catNav=document.getElementById('kt-category-nav');if(!catNav)return;catNav.style.top=(nav?Math.ceil(nav.getBoundingClientRect().height):0)+'px';}
  function categories(ps){const list=[...new Set(ps.map(cat))].sort((a,b)=>a.localeCompare(b));const strip=document.getElementById('kt-category-strip');if(!strip)return;const activeCat=currentActiveCat();strip.innerHTML=['All',...list].map(c=>`<button type="button" class="kt-cat ${activeCat.toLowerCase()===c.toLowerCase()?'active':''}" data-cat="${esc(c)}">${esc(catTitle(c))}</button>`).join('');strip.querySelectorAll('.kt-cat').forEach(b=>b.addEventListener('click',()=>{state.category=b.dataset.cat;state.activeSection=b.dataset.cat==='All'?'All':null;render();}));const shareBtn=document.getElementById('kt-share-category');if(shareBtn){if(state.category==='All'){shareBtn.hidden=true;}else{shareBtn.hidden=false;shareBtn.textContent='Share "'+catTitle(state.category)+'"';shareBtn.dataset.cat=state.category;}if(!shareBtn.dataset.bound){shareBtn.dataset.bound='1';shareBtn.addEventListener('click',(ev)=>{if(typeof shareCategory==='function'&&shareBtn.dataset.cat)shareCategory(ev,shareBtn.dataset.cat);});}}const coldNote=document.getElementById('kt-cold-chain-note');if(coldNote)coldNote.classList.toggle('show',activeCat.toLowerCase()==='vaccines');const prev=document.getElementById('kt-cat-prev'),next=document.getElementById('kt-cat-next');const sync=()=>{const overflow=strip.scrollWidth>strip.clientWidth+2;if(prev)prev.hidden=!overflow;if(next)next.hidden=!overflow;if(overflow){if(prev)prev.disabled=strip.scrollLeft<=2;if(next)next.disabled=strip.scrollLeft+strip.clientWidth>=strip.scrollWidth-2;}};if(prev&&!prev.dataset.bound){prev.dataset.bound='1';prev.addEventListener('click',()=>strip.scrollBy({left:-Math.max(180,strip.clientWidth*.65),behavior:'smooth'}));}if(next&&!next.dataset.bound){next.dataset.bound='1';next.addEventListener('click',()=>strip.scrollBy({left:Math.max(180,strip.clientWidth*.65),behavior:'smooth'}));}if(!strip.dataset.arrowBound){strip.dataset.arrowBound='1';strip.addEventListener('scroll',sync,{passive:true});window.addEventListener('resize',sync);}requestAnimationFrame(sync);}
  ...
  function render(){const ps=products();if(!ps.length)return;ensureControls();syncStickyOffset();categories(ps);const visible=filtered(ps),root=document.getElementById('kt-home-catalogue');if(!root)return;if(!visible.length){root.innerHTML='<div class="kt-empty">No products match your search. Try another product name, brand or category.</div>';}else{const catOrder=[...new Set(visible.map(cat))].sort((a,b)=>a.localeCompare(b));root.innerHTML=catOrder.map(c=>{const items=visible.filter(p=>cat(p)===c);return `<section class="kt-cat-section" data-cat="${esc(c)}"><div class="kt-cat-divider" data-cat="${esc(c)}">${esc(catTitle(c))}</div><div class="kt-grid">${items.map(card).join('')}</div></section>`;}).join('');}root.querySelectorAll('.kt-card').forEach(el=>{const p=ps.find(x=>x.name===el.dataset.name);if(!p)return;el.querySelectorAll('.kt-pack').forEach(b=>b.addEventListener('click',()=>{el.querySelectorAll('.kt-pack').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');}));el.querySelectorAll('.kt-qty-btn').forEach(b=>b.addEventListener('click',()=>{const m=minQty(p),cur=state.qty[key(p)]||m;state.qty[key(p)]=b.dataset.dir==='+'?cur+1:Math.max(m,cur-1);el.querySelector('.kt-qty-value').textContent=state.qty[key(p)];}));el.querySelector('.kt-add')?.addEventListener('click',()=>add(p,el));el.querySelector('.kt-view')?.addEventListener('click',()=>{if(typeof openProductByName==='function')openProductByName(p.name);});el.querySelector('.kt-share')?.addEventListener('click',(ev)=>{if(typeof shareProductNative==='function')shareProductNative(ev,p.name);});});updateOrderBar();computeActiveSection();}
  ...
  function install(){if(state.installed)return;state.installed=true;installStyles();ensureControls();syncStickyOffset();let spyTicking=false;window.addEventListener('scroll',()=>{if(!spyTicking){spyTicking=true;requestAnimationFrame(()=>{spyTicking=false;computeActiveSection();});}},{passive:true});window.addEventListener('resize',()=>{syncStickyOffset();computeActiveSection();});const legacy=window.addToCart;window.addToCart=function(name,pack){legacy(name,pack);if(document.getElementById('cart-drawer')?.classList.contains('open'))closeCart();updateOrderBar();};const oldQty=window.updateCartQty;window.updateCartQty=function(i,q){oldQty(i,q);updateOrderBar();};const oldRemove=window.removeFromCart;window.removeFromCart=function(i){oldRemove(i);updateOrderBar();};const oldClear=window.clearCart;window.clearCart=function(){oldClear();updateOrderBar();};render();}
  let tries=0,shellReady=false;function boot(){if(!isHome())return;if(!shellReady){shellReady=true;installStyles();ensureControls();syncStickyOffset();const root=document.getElementById('kt-home-catalogue');if(root)root.innerHTML='<div class="kt-loading"><i class="fas fa-circle-notch fa-spin"></i><span>Loading products…</span></div>';}if(products().length){install();return;}if(++tries<400)setTimeout(boot,50);}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
})();
